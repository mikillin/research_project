...
% ****************************** TOP MATTER ***********************************
\renewcommand{\author}{Sergey Rogachevsky}           % Name
\newcommand{\dateOfBirth}{1981-10-10}           % Geburtsdatum
\newcommand{\matrNumber}{710 4711}              % Matrikelnummer
\newcommand{\studycourse}{Computer Science}           % Studiengang
\newcommand{\specialization}{Embedded Systems Engineering}           % Studiengang

\newcommand{\supervisor}{Prof. Dr.-Ing. Christoph M. Friedrich} % Betreuer

\newcommand{\secondsupervisor}{M.Sc. Bastian Dewitz} % Zweitbetreuer
...
\newcommand{\subject}{Research Project}  % Art/Thema der Arbeit
\newcommand{\titel}{Development of a Multi-Camera Video Acquisition and Synchronization \\ Module for ICU Digital Twin Systems \\ } % Titel der Arbeit
\newcommand{\subtitel}{} % Untertitel
\newcommand{\degree}{Master of Engineering} % Angestrebter Titel (nur bei Abschlussarbeiten, sonst leer lassen/auskommentieren)

\newcommand{\keywords}{Research project, Informatik, {FH Dortmund}} % Stichworte (durch Komma getrennt)
...
\begin{document}
	...
	\section*{Abstract}
	ICU ist eine Moderne Technologie, die eröffnet neue GElegentheiten für Beobachtung, Verwaltung, Analyse verschiedene Objekten. Das medizinische Bereich verlängt immer die Präzise und stabil Entscheidungen. Obwohl die Technologien und Standarten werden immer  verbessert, bleiben medizinische Fehler immer noch auf die Höhe Stufe. Mit Entwicklung neue Technologien neue Vorgehen sollten schnellmöglichst umgesetzt werden, weile s verschiedene Leben reteten kann. Sogat einfache Checklist können die Qulität der Arbeit den medizinischen Spezielisten verbessern. Computer Science aber biettet besseren methodologien an, die die Ergebnisse des Behandlungs in Kliniken  stark erhöhen kann. 
	In diesem Projekt ist vorgesehen zu enwiklen ein Modul, der auf einen EDGE Gerät installiert sein sollte. Das modul muss von mehrere VCAmeras das Bild aufnehmen, die bestimmte Objekte erkennen und die bestiimte Informationen darüber zu übergeben.
	Dies sollte als ein Teil zu moderne ICU eines Klinikums sein. Das muss entsprechen, den mehreren unterschiedlichen Anforderungen zu dem System und software, die in deisem Projekt von DIN sowie FDA eingenommen werden. 
...
With development LLM technologies the requirements from the world about truth events inceresed tremendously. Differrent mthodes, new technologies makes infmormation overflowing. new approaches should be investigated and implemented  for the better results. 
Among surveilence technologies can be used dieffernet sensors: ultrasound, infrared, LIDAR, For taking a correct decision they can be used together as sensor fusion. In this project it was defined reuiqrements to detect objects, their positions, defined if their position is inside defiened area and detect humans activites if someone is in the ICU. This information should be savedin appropriate form for future analysis.
The sensors for this research projects were defined as RGBD cameras, promarily Femto Mega ORBBEC. The purpotse of the project is to assess accuracy and overall performance of such survilence system and prepare it to be in accordance to the law regulatory and medical standards in the software and hardware development. The project should extract the most reasonable parameters of the surveilence system and assess the developed plattform. the palttform should be later become a base for development Digital Twin for ICU.
	\newpage
	\section*{Abstract}
Hier findet eine deutsche Übersetzung des englischen \glqq Abstract\grqq{} statt. 
	...
	die Role des ICUs in Kliniken kann nicht übersteigert sein: retten des Menschens vordert höhe Afinität zu präzise und stabiele Arbeit. Obwpohl sind die Spezielisten von sehr höhre Qualität und haben mehrjahrige Erfahrung haben passieren die Fehler immer noch. Dies kann zu schwache Konsequenzen führen, sogar zum Tod den Patienten. 
	\section{Motivation}
	...
	Das Projekt ist eingerichtet, das Prototyp von DIgitalle Zwilling zu entwicklen. DAs Prototayp muss skalierbarkeit ausdrucken.  
	...
	Digital twin became a novice but thogh evry poluar methodology, due to its advamtages.  As a novel methodology which may significatly impact  on the medicie lot of large companies started to work on their own plattofrms. Planning future surgeries, simulation of the functionalities, induce  personalized treatment through artificial models, simulating variety of deseases to develpe new approasches in threatment and reposnse of organs on it.  Complexity of such system lead to division into as layers so architectures. Some companies concetraited their efforts on small critical parts as for example  till  for the entire patinet models with integrated EHR, genomics and imaging, and DigiTwins (EU Horizon 2020 project) oriented on predicting health outcomesand guiding personalized medidcine.
	
	- Simulated distinctive Organs
		Philips Digital Twin for Cardiology or Siemens Healthineers - Digital Twin of the heart, Dassauot Systemes - Libig Heart and Living Lung Project
	- Body-leveld Systems
	Siemens Healthineers and IBM Watson Health
	- Dsease specific Plattforms
	nvidia Clara project for the cancer genome analysis
	- Surgery and rehabilitationb Twins
	intuitive Surgical (Da Vinci), Rehabilitation twin to simulate musculoskeletal functions.
	- Infrastructure and Device-level Twins
	GE Healthcare and Medtronic - medical devices, ICU Bed Digital Twin
...
The Software Bill of Materials is a list of the software used for the implementation and its licenses. An example is shown in Table \ref{TAB:Software}.
	...
	Currently the amount of new languages so as old enhanced are overwelming. As in this application will be used  2 parts, it was decided to use for backend: python and java.frontend python TK framework and vue.js.
	Although current situation in programming languages area is very dynamic: some languages quite fast become obsolete, some revive from the post, some environmental requirements limited the selecion of them. Thus, as a fronend to the TK python framework. TK framework is one threaded simple framework, oriented onto fast modeling of an application using python programming lagnuage. This framework has a set of GUI elements, which mostly are enough to bild user-friendly GUI. As in the app it is used machine learning python was chosen as a perfect tool for that, makine development of the part of application easier, faster and more cofrtable. For the backend it was used spring boot.  Object oriented language make it better to create applications with complex data structure. fronend for the admin site was selected vue.js. This is a modern framework makeing UI flexible, scalebale and user friendly. As A student of FH Dortmund it was clearly selected JetBrains PyCharm and JetBrains IntellyJ Idea. Thses IDE are used on all levels of developemtn - from students till professionnals, because of wide programming features. The used license is from FH Dortmnd, what make it free for students. 
	Libraries which were used are:
	o3d. IT is open source library. 
